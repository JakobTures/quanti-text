...
einem Projket arbeiten und möchten eventuell ihren code mit Kommiliton:innen,
...
Dies sind einige der Argumente die dafür sprechen, mit R scripts statt direkt in
der console zu arbeiten.
...
oder Veränderungen vorzunehmen. Umsetzen könenn Sie dies mit einem Klick auf
"Run" in der toolbar des script Fensters oder noch einfacher durch das Drücken
von "CTRL" + "Enter". In beiden Fällen wird die Zeile in der sich aktuell Ihr
cursor befindet ausgeführt. Dieser springt daraufhin automatisch in die nächste
Zeile, so dass sie auch Befehl für Befehl schnell ihr ganzes script ausführen
...
Sie aber immer, dass R zu jeden Zeitpunkt davon ausgeht, dass Sie wissen was Sie
...
Arbeitssession beenden, sondern regelmäßig. Dies können sie über "File" > "Save"
erreichen oder durch die Tastenkombination "CTRL" + "S". Sie können den Namen
des scripts frei wählen, die Endung muss jedoch immer ".R" sein.
...
dem `+` den code fortsetzen können. In der Regel ist es am praktikabelsten die
Ausführung des codes abzubrechen indem sie in die console klicken und danach
...
Einen Kommentar markieren wir mit einem oder mehreren `#` zu Beginn einer Zeile.
Text der auf ein `#` folgt wird von R als Kommentar erkannt und entsprechend
nicht ausgeführt.
...
weiterzugeben und davon abzusehen, in die Datenstruktur von anderen Personen
einzugreifen.

### Speichern

Aus meiner Sicht ist es aus dem Blickwinkel reproduzierbarer Datenanalyse meist
...
dem Namen bzw. Pfad der Datei als einziges Argument.
...
transportierbaren Format zu speichern, biete sich ".csv" an. Mehr dazu hier:
...
sehr schnell auch weitere tidyverse packages verstehen werden. Dies ist
letztlich Geschmackssache, aber aus meiner Sicht führt das Arbeiten mit dem
tidyverse zu intuitiverem und flüßiger zu schreibendem R code, vor allem dank
der pipe `%>%`, die im nächsten Kapitel eingeführt wird.
...
aber ungewohnt erscheint, da wir nicht daran gewohnt sind Daten in dieser Form
darzustellen. Dies erfordert zu Beginn etwas Gewöhnung. Wir sollten aber auch
bedenken, dass es an dieser Stelle nicht das Ziel ist eine schöne Tabelle zu
präsentieren sondern ein Datenobjekt zu konstruieren welches uns die praktische
und komfortabele Weiterarbeit mit den diversen tidyverse packages ermöglicht.
...
Einführung finden Sie in "R for Data Science":
...
aber die Funktion `tibble()`. Dazu müssen wir aber zunächst das **tibble**
...
Version des vorangegangenen codes könne wie folgt aussehen:
...
Folgender code hat das Ziel, für einen vektor von Zahlen einen auf eine
...
sie angwandt werden sollen als erstes Argument nehmen, was sehr häufig der Fall
...
Dazu schreiben wir einfach ein `?` vor den Namen einer Funktion zu der wir Hilfe
benötigen. Die zugehörige Hilfedatei öffnet sich dann im "Help" Tab, 
rechts-unten. Versuchen wir dies für die Funktion `rnorm()`.
...
Eine ausführlichere Dokumentation zu einem package, finden Sie meist auf der
dazugehörigen CRAN Seite, hier z.B. für **tibble**:
