...
Dieses Kapitel behandelt die Bereinigung, Transformation und deskriptive
statistische Analyse von Daten mit **dplyr**, sowie deren graphische Analyse mit
**ggplot2**. Zur Veranschaulichung der Arbeitsschritte, analysieren wir eine
Auswahl biographischer Daten der Bundestagsabgeordneten des 19. Bundestags.
Darunter auch die Regierungsmitglieder und Funktionsträger wie
...
Das script speichert den tibble in der Datei "stammdaten.RData", die von
...
handelt, müssen wir auch zunächst das **tidyverse** package laden. R kennt ohne
dieses keine tibble und interpretiert das Objekt als data frame.
...
Die Datei enthielt als Einziges das Objekt `stammdaten`, einen tibble der die
ausgewählten Stammdaten aller jetzigen und ehemaligen Bundestagsabgeordneten
enthält. Schauen wir uns den tibble einmal an:

``` {r print_stammdaten}
stammdaten
```

Der tibble enthält 4089 Zeilen, also Daten zu 4089 Abgeordneten. Zu jeder
Beobachtung, stehen 9 Variablen bereit. Nachname, Vorname, akademischer Titel,
Geschlecht, Parteizugehörigkeit, die erste und letzte Wahlperiode in der das MdB
im Bundestag vertreten war sowie Alter und Geburtsdatum.
...
Schreiben von `View(objektname)` in der console. Nun öffnet sich das Objekt in
...
`print()` möglich. Als zweites Argument können wir mir `n = ` angeben, wie viele
Zeilen wir sehen möchten.
...
`filter()` dient dazu, Beobachtungen -- also Zeilen -- aus einem tibble zu
...
sind letztlich die Regeln, nach denen die Funktion filtern soll.

Zum Schreiben der Ausdrücke benötigen wir *vergleichende Operatoren*. Diese 
vergleichen den Wert auf der linken Seite des Operators mit dem auf der rechten,
und geben als Ergebnis ein `TRUE` zurück wenn der Test bestanden wurde
bwziehungsweise `FALSE` wenn er nicht bestanden wurde.

Um zu testen ob zwei Werte gleich sind, nutzen wir den Operator `==`. Bitte
beachten Sie, dass wir zwei Gleichheitszeichen nutzen müssen. `==` vergleicht
zwei Werte, während `=` den Wert auf der linken Seite mit dem auf der rechten
...
Ebenfalls können wir vergleichen ob der linke Werte kleiner `<`, kleiner-gleich
`<=`, größer `>` oder größer-gleich `>=` ist.
...
heraus. Wir können dazu in `filter()` testen, ob der Wert der Spalte "Partei"
gleich dem Wert "CDU" ist.
...
filtern, also CDU- und CSU-Mitglieder, können wir dies mit `|` realisieren:
...
möchten Die Spalten "nachname", "vorname", "Titel" und "Partei" extrahieren:
...
Aktuell ist der Name über zwei Spalten verteilt. Möchten wir stattdessen eine
Spalte mit dem kompletten Namen erstellen, können wir dies mit `mutate()`
umsetzen. Generell schreiben wir innerhalb der Klammern den Namen der zu
transformierenden oder neu zu erstellenden Spalte `=` einem Ausdruck der
...
interessiert uns nur ob ein Titel vorliegt, aber nicht welcher. Dazu könnten wir
eine neue Spalte generieren, welche einen logischen Vektor enthält der angibt ob
ein Titel vorliegt, `TRUE`, oder nicht, `FALSE`. Dies ist eine sogenannte
Dummyvariable. Zum generieren können wir die **dplyr** Funktion `if_else()`
nutzen. Wir geben als erstes Argument eine Bedingung an die getestet wird. Hier
`!is.na(titel)`, welche testet ob der Inhalt von "titel" *nicht* -- angezeigt
durch das `!` -- missing ist. Das zweite und dritte Argument geben an, was
eingesetzt werden soll wenn die getestete Bedingung erfüllt beziehungsweise
...
Die Spalte "partei" ist auch als character vector abgelegt. Bei der
...
tidyverse package **forcats** ermöglicht es uns Faktorvariablen anzulegen und zu
...
Die Spalte "geschlecht" liegt ebenfalls als character vector vor und kann auf
...
einem tibble zu berechnen. Die Syntax funktioniert dabei ähnlich zu `mutate()`.
...
Ergebnis ist ein neuer tibble der die angeforderten Statistiken enthält.
...
Bundestag vertreten sind. Dazu nutzen wir innerhalb von `mutate()` die **dplyr**
Funktion `n()`, welche die Gruppengröße, also die Anzahl Beobachtungen pro
Gruppe, zurückgibt.
...
Weiter oben haben wir das Durchschnittsalter aller Abgeordneten berechnet. Dies
können wir natürlich auch gruppiert nach der Partei machen.
...
in einem weiteren Schritt mit der Funktion `arrange()` nach den Werten einer
...
einen neidrigeren Median. Wir könnten also vermuten, dass in der AfD einige
...
Diesen ersten Befunden werden wir bei der graphischen Analyse
(Kapitel \@ref(R4)) weiter auf den Grund gehen. 
...
absolute Anzahl an Titeln zu berechenn, reicht es also die Summe des Vektors zu
bilden. Alle Werte werden so aufaddiert und da wir nur Einsen und Nullen haben,
ist das Ergebnis gleich der Anzahl der Abgeordneten die einen akademischen Titel
...
hhaben, bedeutet dies also, dass die Daten nach dem `summarise()` Befehl nicht
...
Das Exportieren von Tabellen -- beispielsweise die oben berechneten Statistiken
...
Normalfall möglich sein. 

*CSV* steht für "comma-separated values". Da diese sehr einfach gehalten sind
und von einer großen Bandbreite an Software verarbeitet werden können, ist dies
eines der meistgenutzten Datenformate.

Die Grundstruktur ist dabei wie folgt:
...
So können wir beispielsweise das Ergebnis einer `summarise()` Operation mit
...
Aus Gründen die vermutlich nur Microsoft selbst bekannt sind, können ".csv"
...
mit `write_csv2()` auf einfachem Wege möglich..

Mehr zu ".csv" Dateien finden sie unter:
...
**huxtable** kann Tabellen im ".tex" Foramt zur Arbeit in LaTeX ausgeben:
<https://hughjonesd.github.io/huxtable/>{target="_blank"}

Möchten sie externe Software zum Schreiben von Hausarbieten und Papers ganz
...
Die Website die Sies aktuell betrachten sowie viele der verlinkten Resourcen
