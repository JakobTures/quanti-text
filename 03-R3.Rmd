...
Bundestag vertreten war sowie Alter und Geburtsdatum.
...
kombinieren. Das optionale Argument `sep = ` wird dazu genutzt festzulegen,
...
`NA`, die Repräsentation fehlender Werte in R. Für diese Analyse interessiert
...
vorliegt, `TRUE`, oder nicht, `FALSE`. Dies ist eine sogenannte Dummyvariable.
Zum generieren können wir die **dplyr** Funktion `if_else()` nutzen. Wir geben
als erstes Argument eine Bedingung an die getestet wird. Hier `!is.na(titel)`,
was testet ob der Inhalt von "titel" *nicht* -- angezeigt durch das `!` --
...
soll im ersten Fall `TRUE`, im zweiten `FALSE` eingesetzt werden.
...
Ordnung anhand der größe der Fraktionen im 19. Bundestag.
...
* Das R Markdown cheat sheet: <https://raw.githubusercontent.com/rstudio/cheatsheets/master/rmarkdown-2.0.pdf>{target="_blank"}
