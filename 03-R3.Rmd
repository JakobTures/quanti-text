...
deskriptive statistische Analyse von Daten mit **dplyr** sowie der graphische
...
auch die Regierungsmitglieder und Funktionsträger wie Bundestagspräsident*innen.
Die Daten stammen aus einer vom Bundestag bereitgestellten ".xml" Datei und
wurden am 07.09.2021 unter folgender URL heruntergeladen:
...
Das script speichert einen tibble mit allen genutzten Daten in der Datei
"stammdaten.RData", die von Seminarteilnehmern auch in Moodle heruntergeladen
werden kann. Laden wir diese Daten zunächst in R. Da es sich bei dem Datenobjekt
um einen tibble handelt, müssen wir auch zunächst das **tidyverse** package
laden. R kennt ohne dieses keine tibbles und interpretiert das Objekt als data
frame.
...
`TRUE` zurück wenn der Test bestanden wurde bwziehungsweise `FALSE` wenn er
...
Das Zuammenfassen mehrer factors wäre auch mit `fct_recode()` möglich, erfordert
aber etwas mehr Schreibart. Diese Funktion eignet sich aber zum Anpassen der
Namen einzelner factors.
...
sondern auch fehleranfällig, vor allem bei umfassenderen Analysen als solche,
die wir bisher betrachet haben.

Ein flexibler Weg zum direkten Export von Tabellen ist es, diese als ".csv"
abzuspeichern. Dieses Format kann wiedrum direkt in Excel und vermutlich auch in
...
Kommas. In der ersten Zeile stehen dabei häufig, aber nicht immer, die
...
Spalten durch Kommas getrennt sind -- der internationale Standard -- auf
deutschen Windows Systemen nicht ohne weiteres importiert werden. Nutzen Sie
statt Microsoft Office die open source Alternative Libre Office, ist das Problem
...
statt Kommas Semikolons als Trennzeichen der Spalten verwendet da die Kommas in
Deutschland zur Darstellung von Dezimalzahlen benötigt werden. Dies ist mit
