...
In diesem Kurs werden wir mit der Sprache R in Kombination mit der IDE RStudio
arbeiten. Bevor wir mit der Einführung in die grundlegende Bedienung beginnen,
zunächst einige Worte zur Installation der kostenlos zugänglichen Software.

### R

*R* ist eine Programmiersprache die vor allem für statistische Analysen und data
science angewandt wird. Generelle Informationen zu R und die ausführliche
Dokumentation finden sie unter: <https://www.r-project.org/>{target="_blank"}.

Die aktuelleste Version von R für alle gängigen Betriebssysteme findet sich
stets im *CRAN*, dem *Comprehensive R Archive Network* unter:
...
In beiden Fällen führen Sie bitte die passende Datei aus und Installieren Sie R
in ein Verzeichnis Ihrer Wahl. Für Linux Systeme stellt CRAN
Installationsanweisungen für einige der populäreren Distribution bereit.
...
Benutzeroberfläche. Das populärste IDE ist *RStudio*, zu finden unter:
<https://www.rstudio.com/>{target="_blank"}. Unter "Download" wählen Sie bitte
...
mit der Notation `x^y`. Im folgenden code Block finden Sie einige Beispiele, die
...
Geschmackssache, es empfielt sich aber eine einheitliche Schreibweise sowie
kurze aber klar verständliche Namen zu verwenden. Daten weisen wir einem Objekt
mit dem *assignment operator* `<-` zu.
...
So erstelle Objekte werden in dem "Environment" Tab (rechts-oben) gelistet.
...
Hier handelt es sich um einen numerischen Vektor -- *numeric*, ausgedrücht durch
...
ausgelesen, als character Vektor gespeichert haben.
...
Wir können data frames auch aus zuvor definierten Vektoren zusammensetzen.
...
Da data frames zweidimensional sind, müssen wir beim subsetting auch eine
...
Die R Welt ist offen und kollaborativ. Neben den base R packages die in der
...
aber auch still, ohne Informationen in die console zu schreiben. Sollte beim
...
ohne erweiterte Programmierkenntnisse dazu in der Lage wären.
...
Weitere Beispiele für Funktionen sind einige der in base R enthaltenen Methoden
statistischen Maßzahlen zu berechnen. Legen wir zunächst einen numerischen
Vektor mit einigen Beispieldaten an:

``` {r numeric_exmpl_data}
data <- c(4, 8, 15, 16, 23, 42)
```

Unsere Interesse sei es das arithmetische Mittel, den Median sowie die
